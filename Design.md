...
if Math.random>0.7
    ...
    print â€œYou search the tree for fruit, but you canâ€™t find any ripe ones.â€�
~~~
...
* Create 3 new instance variables respectively in Stegosaur and Allosaur class:
	...
	* an int instance variable called **starvationLevel** , its initial value is 0. When foodLevel == 0 , starvation will increase 1 every turn, if foodLevel >0 , starvation will set to zero again. This variable is used to track how many turns the dinosaur are in starving state (foodLevel ==0).
...
- After 20 turns , the dinosaur.starvationLevel  == 20 , call out map.removeActor(dinosaur) to remove the dinosaur from the game map to show that the dinosaur is dead.
...
- If the foodLevel is lower than 30, the dinosaur will become hungry and it's behaviour instance variable will be set as hungryBehaviour.
...
- Create a functions in Stegosaur class to implement its eat feature:
	...
	* eatMealKit(Item mealkit) : this function is used for stegosaur to eat mealkit. add 100 to the dinosaur foodLevel per mealkit eaten.

- Create a functions in Allosaur class to implement its eat feature:
	* eatDino(Stegosaur dianosaurMeat) : this function is used for allosaur to eat stegosaur .
	
 	* eatCorpse(Corpse dianosaurMeat) : this function is used for allosaur to eat corpse .

   	* eatEgg(Egg egg) : this function is used for allosaur to eat egg .
...
- Create a function called **isFoodAroundMe(Actor,Map)** , check if any food adjacent to the dinosaur then return true if it has
...
- Create 2 new instance variables in Stegosaur and Allosaur classes : 
 	...
 	1. Created an Enum class Gender to represent the gender of dinosaur Male/ Female 

	2. Created an Enum class LifeStage to represent the life stage of dinosaur Adult/Child/Pregnant
...
- The hatching of egg will be done in Egg class, in the playTurn method. Egg will be hatched after 20 turns of game

- Operation to allow baby dinosaur to grow to become adult will be handle in the updateState method inside dinosaur classes by using age.
...
**Operation of the BreedingBehaviour class:**
 ...
 - When female dinasaur behaviour set as BreedingBehaviour , they will started searching if there is an opposite sex but same species dinasaur is adjacent to it. Once it found, the female dinosaur will become prenant if there's an opposite gender dinosaur and same species with it which lead to mating success.

 - The initial implementation of the looking partner algorithm will be similar to the looking foodSource algorithm mentioned above in the hungry feature design.First it will look for partner adjacent to the dinosaur, then only start search from map if there's no any suitable partner adjacent to it. It look for the same species dinosaur by searching from the gameMap by looping tru the GameMap Then, we apply the FollowBehaviour method by moving the female dinosaur one step closer to it's partner location.

 - Only when dinosaur.lifeStage == "adult" && dinosaur.foodLevel > 50 :  behaviour can be set as  BreedingBehaviour.
...
- A children class of Actor to represent carnivore type dinosaur
- Class instance variables: 
	...
	* Int starvationLevel : represent the the time the allosaur inconscious due to hunger
...
- Functions : 
	...
	* eatMealkit (Mentioned above in the hungry feature implementation)
	* eatCorpse  (Mentioned above in the hungry feature implementation)
	* becomeAdult (Mentioned above in the breeding feature implementation)


#Extra featires created in Assignment2 : 
